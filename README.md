...
- 🖱️ **System tray support**: Cross-platform tray icons with custom menus and notifications
...
Tronbun provides comprehensive system tray support with custom menus, notifications, and event handling across all platforms (Windows, macOS, Linux).
...
Show native system notifications from the tray:

```typescript
await tray.showNotification("Title", "Notification message");
```
...
- **macOS**: Uses NSStatusItem with native menu support and UserNotifications
- **Windows**: Uses Shell_NotifyIcon with popup menus and balloon tooltips
- **Linux**: Uses GTK StatusIcon with libnotify for notifications
